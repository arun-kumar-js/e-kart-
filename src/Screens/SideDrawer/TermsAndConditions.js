...
const TermsAndConditions = () => {
  console.log('TermsAndConditions component mounted');
  ...
  const fetchTerms = async () => {
    ...
    try {
      ...
      if (response.data && !response.data.error) {
        console.log('API response terms:', response.data.terms);
        setApiResponse(
          typeof response.data.terms === 'string'
            ? response.data.terms
            : JSON.stringify(response.data.terms),
        );
      } else {
