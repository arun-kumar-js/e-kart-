...
const Cart = ({ navigation }) => {
  ...
  const renderItem = ({ item }) => {
    const name = item.variants && item.variants.length > 0 ? item.variants[0].product_name || item.name : item.name;
    const measurementUnit = item.variants && item.variants.length > 0 ? item.variants[0].measurement_unit_name || '-' : '-';
    ...
    return (
      <View style={styles.itemContainer}>
        ...
        <View style={styles.detailsContainer}>
          ...
          <Text style={commonTextStyles.description}>Qty: {measurementUnit}</Text>
  ...
  const totalPrice = cartItems.reduce(
    (sum, item) =>
      sum +
      (item.quantity || 0) * (item.price || item.variants[0].product_price||0),
    0,
  );
          ...
          <View style={styles.bottomContainer}>
            ...
            <View style={styles.checkoutFooter}>
              <Text style={styles.checkoutText}>
                Total {itemCount} Items RM {totalPrice ? totalPrice.toFixed(2) : '0.00'}
              </Text>
